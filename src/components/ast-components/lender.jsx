...
export default function Lender({ data, context }) {
  ...
  const handleClickYes = (path) => {
    dispatch({
      type: 'SET_LENDER',
      value: { ...lead, isMilitaryLenderReq: true }
    });
    navigate(path);
  };
  ...
  const handleClickNo = (path) => {
    dispatch({
      type: 'SET_LENDER',
      value: { ...lead, isMilitaryLenderReq: false }
    });
    navigate(path);
  };
        ...
        <>
          <div className={classes.root}>
            ...
            >
              ...
              <Grid item>
                <Button
                  onClick={() => handleClickNo(lead.operationType === 'BUYSELL' ? `/signup/${context.next.screenType.toLowerCase().replace(/\s/g, '')}` : `/signup/${lead?.operationType?.toLowerCase().replace(/\s/g, '')}/${context.next.screenType.toLowerCase().replace(/\s/g, '')}`)}
                  variant="contained"
                  className={clsx(globalThemeCss.button, classes.nextButton)}
                >
                  No
